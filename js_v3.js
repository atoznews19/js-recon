...
// @version      3.0
// @description  Recon scanner for authorized web testing: endpoints, GraphQL, WebSockets, secrets, S3, subdomains, and bookmarks.
// @match        *://*/*
// @grant        none
// @run-at       document-end
// ==/UserScript==
...
function createScanButton() {
    if (document.getElementById("__recon_scan_btn")) return;

    var btn = document.createElement("button");
    btn.id = "__recon_scan_btn";
    btn.innerText = "🔍 JS Recon";
    ...
    btn.onclick = function () {
        ...
        // Gather script sources from current page
        D.sources = Array.from(document.querySelectorAll("script[src]"))
        ...
        D.total = D.sources.length;
        D.scanned = 0;

        // Show the recon UI
        var reconEl = document.getElementById("__recon__");
        if (reconEl) reconEl.style.display = "flex";

        // Run fetches, re-enable button when done
        runFetches(function() {
...
/* ── Initialise: build UI, hide it, and show scan button ────── */
var ui = buildUI();
document.getElementById("__recon__").style.display = "none";
createScanButton();
